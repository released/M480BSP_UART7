...
#define FLAG_PROJ_TRIG_1                 				(flag_PROJ_CTL.bit2)
...
void UARTx_SendByte(UART_T *uart ,unsigned char ch)
{
    while(UART_GET_TX_FULL(uart));
    UART_WRITE(uart,ch);
}

void UARTx_SendMultiBytes(UART_T *uart ,unsigned char* buffer ,unsigned int len)
{
    unsigned int i = 0;
    for (i = 0 ; i < len ; i++)
    {
        UARTx_SendByte(uart,buffer[i]);
    }
}

void UARTx_polling(void)
{
    unsigned char buffer[16] = {0};
    unsigned char i = 0;
    static unsigned char cnt = 0;
    unsigned char len = sizeof(buffer) / sizeof(buffer[0]);
    ...
    UARTx_SendMultiBytes(UART7, buffer, len);
...
void UART7_Init(void)
{
    ...
    /* Configure UART7 and set UART0 baud rate */
    UART_Open(UART7, 115200);

    UARTx_SendByte(UART7, 0xC3);
    UARTx_SendByte(UART7, 0x3C);
    UARTx_SendByte(UART7, 0xF0);
    UARTx_SendByte(UART7, 0x0F);
}
...
//    CLK_WaitClockReady(CLK_STATUS_LXTSTB_Msk);
    ...
    SYS->GPH_MFPL |= (SYS_GPH_MFPL_PH7MFP_UART7_RXD | SYS_GPH_MFPL_PH6MFP_UART7_TXD);
